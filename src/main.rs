...
fn get_traces_for_task(task: &InputTask, path: &str) -> Vec<Trace> {
...
fn main() {
    ...
    let input_button_handler_task = InputTask {
        name: "button_handler".to_owned(),
        interrupt: "IO_IRQ_BANK0".to_owned(),
        priority: 2,
    };

    let path_to_elf_file = "test_bin/rtic_simple_resourse";
    let traces = get_traces_for_task(&input_button_handler_task, path_to_elf_file);
